	...
	if(ad_banner_url == "") {
		if ( keyword == "" ) {
			...
			try {
                ...
                jQuery('#gnbTxtAd').attr('data-log-body', "{'search_keyword':'" + keyword + "'}");
...
HeaderGnb.makeGnb = {
	...
	setMainRightBanner : function(){
		...
		var imgInjection = function(){
			...
			$bcbConts.attr('href',imgLinkArr[currIdx]).attr('data-ga-event-label','기획전배너 클릭_' + gaIdx + '_' +titleArr[currIdx]);
		};
...
HeaderGnb.categoryNavi_v2 = {
	...
	makeGrpCtgrLayer : function(grpIdx) {
		...
		if(grpData != undefined) {
				...
				if( ctgrLv === 1 ) {
					...
					if(isForeign == false){
						...
						if(ctgrNo == 476017){//DLUXURY11 처리
							...
							lv1CtgrLinkUrl = 'http://globalshopping.11st.co.kr/global/GlobalCategoryAction.tmall?method=globalMCtgr&dispCtgrNo=' + ctgrNo;
						}
					}
					
					subCateId = (ctgrNo+'_sub_cate');
					subCateUlId = (ctgrNo+'_sub_cate_'+0);
					$content.append('<li data-log-actionid-area="lnb_nav" data-log-actionid-label="large_category"><a href="' + lv1CtgrLinkUrl + '" class="link_cont" data-ga-event-category="PC_LNB 카테고리 탐색" data-ga-event-action="대카테고리" data-ga-event-label="' + ctgrNm + '" data-ga-Dimension84="PCLNB_TEST_B" data-log-body="{\'content_no\':\''+ ctgrNo +'\', \'content_name\':\''+ ctgrNm +'\', \'content_type\':\'CATEGORY\'}" data-log-index="' +(lv1CtgrCnt+1)+ '">' + ctgrNm + '</a><div class="sub_cate" id="' + subCateId + '" style="width:140px"><ul class="list_sub_cate" id="' + subCateUlId + '"/></div></li>');
					lv1CtgrCnt++;
					lv2CtgrCnt = 0;
			...
			if(mainLnbBanner != undefined && mainLnbBanner.length != 0) {
				...
				$layerCont3.append('<a href="' + data.banner.DispObjLnkUrl + '" class="cate_link" data-ga-event-category="PC_LNB 카테고리 탐색" data-ga-event-action="메타별 배너" data-ga-event-label="' + metaCtgrNm + '_' + data.banner.DispObjNm + '" data-ga-Dimension84="PCLNB_TEST_B"><img src="' + _UPLOAD_URL_ + data.banner.LnkBnnrImgUrl + '" alt="' + data.banner.DispObjNm + '"></a>');
			...
			if(jQuery(lnbMenuLi[grpIdx]).hasClass('recommand')){
				...
				var boxListCnt = 0;
				var recommendCtgrList = eval('FooterData.recommendCtgrList');
				if(recommendCtgrList != undefined) {
					var $recommendCtgrArea = jQuery('<div class="box">');
					for(var idx = 0 ; idx < recommendCtgrList.length ; idx++) {
						$recommendCtgrArea = jQuery('<div class="box">');
						var data = recommendCtgrList[idx];
						var ctgrList = data.ctgrList;
						$recommendCtgrArea.append('<strong class="subject">' + data.title.DispObjNm + '</strong>').append('<div class="cont"/>');
						var recommendDispIndex = 0;
						for(var ctgrIdx = 0 ; ctgrIdx < ctgrList.length ; ctgrIdx++) {
							if(ctgrList[ctgrIdx].DispObjLnkUrl2 != ''){
								...
								jQuery('ul:last-child', $recommendCtgrArea).append('<li data-log-actionid-area="lnb_nav" data-log-actionid-label="alltheme_category"><a href="'+ctgrList[ctgrIdx].DispObjLnkUrl2+'" data-ga-event-category="PC_LNB 카테고리 탐색" data-ga-event-action="추천카테고리_전시구좌" data-ga-event-label="' + ctgrList[ctgrIdx].DispObjNm + '" data-ga-Dimension84="PCLNB_TEST_B" data-log-body="{\'content_no\':\''+ ctgrList[ctgrIdx].DispObjNo +'\', \'content_name\':\''+ ctgrList[ctgrIdx].DispObjNm +'\', \'content_type\':\'CATEGORY\'}" data-log-index="' +(ctgrIdx+1)+ '"><span class="thumb"><img src="'+ _UPLOAD_URL_ + ctgrList[ctgrIdx].LnkBnnrImgUrl+'" onError="javascript:this.src=\'' +_IMG_URL_+ '/img/prd_size/noimg_100.gif\';" alt=""></span>' + ctgrList[ctgrIdx].DispObjNm + '</a></li>');
							}
						}
	...
	setCtgrNmStyle : function(ctgrInfo, isForeign) {
		...
		if(isForeign){//해외직구 처리
			ctgrLink = 'http://globalshopping.11st.co.kr/global/GlobalCategoryAction.tmall?method=globalSCtgr&dispCtgrNo=' + ctgrNo;
		}else{
