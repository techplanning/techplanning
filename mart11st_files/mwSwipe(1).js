	...
	var touchEvent = {
		...
		touchend: function(e) {
			...
			if (!isScrolling) {
				...
				var actionStatus = {"isValidSlide":isValidSlide, "direct":direct};

				options.onTouchEnd && options.onTouchEnd(e, view[liveIdx], dataSetIdx, actionStatus);
			}
	...
	function init() {
		...
		if(container) {
			...
			if(view && view.length > 1) {
				...
				if(options.viewSelector == "article" || options.viewSelector == "div"
					|| options.containerSelector == "#dealSwipe>.mwContainer"){  //tab
					...
					var initPosNext = 0;
					var initPosPrev = 0;
					var tempSize = dataset.length;

					for(var temp=0; temp<3; temp++){
						viewTemp[temp] = (dataSetIdx+temp) >= tempSize ? tempFirst++ : (dataSetIdx+temp);
					}
					initPosFirst = viewTemp[1];
					initPosNext = viewTemp[2];
					initPosPrev = viewTemp[0];

			        dataSetIdx = Number(initPosFirst);
                    if(options.isLazyLoad && dataset[initPosFirst] == null) {
                        getLazyDataDirect(initPosFirst,function() {
                        	view[0].innerHTML = dataset[initPosFirst];
							view[0].setAttribute('data-viewIdx', initPosFirst);
							view[1].innerHTML = dataset[initPosNext];
							view[1].setAttribute('data-viewIdx', initPosNext);
							if(view.length > 2) {
								view[2].innerHTML = dataset[initPosPrev];
								view[2].setAttribute('data-viewIdx', initPosPrev);
							}
                            options.onLoad && options.onLoad(view[0], getDataSetIdx());
                        });
                    } else {
		...
		if(options.isLazyLoad) {
			...
			var loadedIndex = [parseInt(options.initPos), nIdx, pIdx];
			for(var i=0,len=options.datasetCount; i<len; i++) {
				if(dataset[i] == null && loadedIndex.indexOf(i) === -1) {
					getLazyDataDirect(i);
				}
